...
// execute in bin: ./Rates idmtheta 
    ...
    long double mtheta, v_h, lam_H, v_s, lam_phi, lam_Hphi, mu_s;
    double fac;
    ...
    v_s = 1000; //395.569 
    lam_phi = 0.1; //0.1
    lam_Hphi = 2.0E-8; //1.0E-6; //
    mu_s = 0.717E-20; 
    long double   mh2 = MH2(lam_Hphi, lam_H, v_s, lam_phi, v_h);
    long double   mH = MH(lam_Hphi, lam_H, v_s, lam_phi, v_h);
    long double   a = Alpha(lam_Hphi,v_s,v_h ,mH,mh2);

    long double mthetaSet[1] = {1.e-20};
    ...
    ofstream outrate (temp);
                    ...
                    myrate.SetParam(mtheta, v_h, lam_H, v_s, lam_phi, lam_Hphi, mu_s);
        

        outrate << "T		" <<"\t"<< "Rate_Total" << "\t" << "LambdaHphi" <<"\t"<<  "Lambdaphi" << "\t" << "LambdaH" << "\t" << "Vsigma" << "\t"<< "Alpha" << "\t"<< "mtheta" <<"\t"<<  "mH2" << "\t" << "mH";
...
// S and T - Channel 
        ...
        myrate.GetProdRate2to2(13,rate13, error13, prob13);

        dec1 = rate1/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec2 = rate2/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec3 = rate3/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec4 = rate4/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec5 = rate5/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec6 = rate6/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec7 = rate7/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec8 = rate8/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec9 = rate9/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec10 = rate10/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec11 = rate11/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec12 = rate12/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        dec13 = rate13/myrate.Get_Nh2(temper[i])/myrate.Get_Hubble(temper[i]);
        
        
        
        rate_total = dec1 + dec2 + dec3 + dec4 + dec5 + dec6 + dec7 + dec8 + dec9 + dec10 + dec11 + dec12 + dec13;

    std::cout << rate_total << std::endl;

        if (outrate.is_open())
        {   
                    
	outrate << std::scientific << temper[i] <<"\t"<< rate_total << "\t" << lam_Hphi <<"\t"<<  lam_phi << "\t" << lam_H << "\t" << v_s << "\t"<< a << "\t"<< mtheta <<"\t"<<  mh2 << "\t" << mH;
                    outrate << "\t" << dec1 << "\t" << error1 << "\t" << prob1;
                    outrate << "\t" << dec2 << "\t" << error2 << "\t" << prob2;
                    outrate << "\t" << dec3 << "\t" << error3 << "\t" << prob3;
                    outrate << "\t" << dec4 << "\t" << error4 << "\t" << prob4;
